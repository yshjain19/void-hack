...
class MockLLMClient(BaseLLMClient):
    ...
    MOCK_RESPONSES = [
        ...
        # 13. System & Application Identity
        {
            "topic": "identity",
            "keywords": [
                "app ka name", "app name", "kya naam", "naam", "name", "who are you",
                "what is this app", "cybertrace", "about", "help", "hi", "hello", "hey",
                "kaun ho", "kya hai", "aapka naam", "tum kaun ho", "application"
            ],
            "reasoning": (
                "Application Name: CyberTrace AI (Enterprise Forensics Suite v2.4).\n\n"
                "CyberTrace AI is an autonomous digital forensics and financial crime investigation platform. "
                "It enables analysts to trace shell company layering, identify strawman nominee directors, "
                "detect circular fund routing (AML), analyze spoofed emails, and generate court-admissible forensic packages."
            ),
            "hypothesis": "The user is querying the name, purpose, or operational identity of CyberTrace AI.",
            ...
            "next_steps": [
                ...
                "Ask: 'Review ISO/IEC 27037 chain-of-custody verification'",
            ],
            "content": "CyberTrace AI platform identity confirmed.",
            "summary": "The application name is CyberTrace AI — an advanced forensic intelligence platform.",
        },
    ]

    async def complete(self, prompt: str) -> dict[str, Any]:
        prompt_lower = prompt.lower()
        # If wrapped with investigation brief, isolate the actual user question
        target = prompt_lower
        if "investigator question:" in prompt_lower:
            target = prompt_lower.split("investigator question:")[-1].split("task:")[0].strip()
        elif "inquiry:" in prompt_lower:
            target = prompt_lower.split("inquiry:")[-1].strip()

        # 1. Match against targeted question first
        for item in self.MOCK_RESPONSES:
            if any(kw in target for kw in item.get("keywords", [])):
                return item

        # 2. Match against full prompt
        for item in self.MOCK_RESPONSES:
            if any(kw in prompt_lower for kw in item.get("keywords", [])):
                return item

        # 3. Fallback to identity or general response instead of random
        for item in self.MOCK_RESPONSES:
            if item.get("topic") == "identity":
                return item
...
class GeminiLLMClient(BaseLLMClient):
    ...
    async def complete(self, prompt: str) -> dict[str, Any]:
        ...
        system_instruction = (
            "You are CyberTrace, an advanced AI digital forensics and financial crime investigator. "
            "The application name is CyberTrace AI. "
            "If the user asks about the app name, identity, or general questions (in English, Hindi, or Hinglish like 'app ka name'), "
            "answer directly and accurately that the application name is CyberTrace AI. "
            ...
            "messages": [
                {
                    "role": "system",
                    "content": (
                        "You are CyberTrace, an advanced AI digital forensics and financial crime investigation platform. "
                        "The application name is CyberTrace AI. "
                        "If the user asks about the app name, identity, or general questions (in English, Hindi, or Hinglish like 'app ka name'), "
                        "answer directly and accurately that the application name is CyberTrace AI. "
                        "Analyze the provided evidence or inquiry and respond in JSON with keys: "
                        "reasoning, hypothesis, confidence, next_steps (list of strings), content, summary."
                    ),
                },
                {"role": "user", "content": prompt},
...
def get_llm_client(
    ...
    if active_key and active_provider in ("mock", ""):
        if active_key.startswith("AIzaSy") or active_key.startswith("AQ."):
